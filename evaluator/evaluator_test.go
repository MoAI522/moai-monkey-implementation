...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		{`first([1, 2], [1, 2])`, "wrong number of arguments. got=2, want=1"},
		{`last([1, 2, 3])`, 3},
		{`last(1)`, "argument to `last` must be ARRAY, got INTEGER"},
		{`last([1, 2], [1, 2])`, "wrong number of arguments. got=2, want=1"},
		{`rest([1, 2, 3])[1]`, 3},
		{`rest([1])`, nil},
		{`rest(1)`, "argument to `rest` must be ARRAY, got INTEGER"},
		{`rest([1, 2], [1, 2])`, "wrong number of arguments. got=2, want=1"},
		{`push([1, 2], 1 + 2)[2]`, 3},
...
func TestBuiltinEvalFunction(t *testing.T) {
	tests := []struct {
		input    string
		expected interface{}
	}{
